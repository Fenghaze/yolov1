...
class detnet_bottleneck(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))   #[n,256,14,14]
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) #[n,64,112,112]

        x = self.layer1(x)  #[n,256,112,112]
        x = self.layer2(x)  #[n,512,56,56]
        x = self.layer3(x)  #[n,1024,28,28]
        x = self.layer4(x)  #[n,2048,14,14]
        x = self.layer5(x)
        # x = self.avgpool(x)
        # x = x.view(x.size(0), -1)
        # x = self.fc(x)
        x = self.conv_end(x)    #[n,30,14,14]
        x = self.bn_end(x)
