...
if __name__ == '__main__':
    ...
    # 测试集label.txt
    f = open('datasets/test_label.txt')
    lines = f.readlines()
    file_list = []
    for line in lines:
    ...
    for image_path in tqdm(image_list):
        result = predict_gpu(model,image_path,root_path='imgs/') #result[[left_up,right_bottom,class_name,image_path],]
        for (x1,y1),(x2,y2),class_name,image_id,prob in result: #image_id is actually image_path
            preds[class_name].append([image_id,prob,x1,y1,x2,y2])
        print(image_path)
        image = cv2.imread('imgs/'+image_path)
        ...
        cv2.imwrite('testimg/'+model_name+image_path,image)
        count += 1
        if count == 100:
            break
